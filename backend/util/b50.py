...
def draw_single_text_border(draw: ImageDraw, font: ImageFont, config, content):
    df = font.font_variant(size=config['font_size'])
...
async def generate_b50_img(play_records: list[PlayRecordInfo], nickname,
                           credential_info: str = "Generated by PRP-API",
                           character: str = 'resources/image/character/para_young_awaken.png',
    ...
    # Draw character:
    character_img = Image.open(character).convert('RGBA')
    character_img = ImageOps.fit(character_img, (config['character']['width'], config['character']['height']))
...
def generate_single(config, font: ImageFont, title_font: ImageFont, record: PlayRecordInfo, index: int, radius=3):
    ...
    single_draw.multiline_text((config['single']['difficulty']['x'], config['single']['difficulty']['y']),
                               text=record.song_level.difficulty + f" {record.song_level.level}",
